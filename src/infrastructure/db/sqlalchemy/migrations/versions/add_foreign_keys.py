...
def upgrade() -> None:
    op.add_column(
        'authorization_codes', sa.Column('session_id', sa.UUID(as_uuid=True), nullable=True)
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        op.f('fk_authorization_codes_user_id_users'), 'authorization_codes', type_='foreignkey'
    )
    op.drop_constraint(
        op.f('fk_authorization_codes_client_id_oauth_clients'),
        'authorization_codes',
    ...
    op.drop_constraint(
        op.f('fk_role_permissions_role_id_roles'), 'role_permissions', type_='foreignkey'
    )
    op.drop_constraint(
        op.f('fk_refresh_tokens_session_id_sessions'), 'refresh_tokens', type_='foreignkey'
    )
