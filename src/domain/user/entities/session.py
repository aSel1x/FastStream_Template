...
class DeviceInfo(BaseValueObject):
    ...
    def with_ip(self, ip_address: str | None) -> DeviceInfo:
        return replace(self, ip_address=ip_address)
...
class SessionAggregate(DomainEventDispatcher, BaseEntity):
    ...
    def find_refresh_token(self, token_hash: TokenHash) -> RefreshToken | None:
        """Any token with this hash, valid or not.

        Revoked tokens have to stay findable: presenting one is how token replay is detected.
        """
    ...
    def find_valid_refresh_token(self, token_hash: TokenHash) -> RefreshToken | None:
        ...
        return token if token is not None and token.is_valid() else None

    def revoke_token(self, token_id: UUID) -> Self:
        new_tokens = tuple(t.revoked() if t.id == token_id else t for t in self.refresh_tokens)
