...
from domain.user.interfaces import CryptInterface, SessionRepositoryInterface, UserRepositoryInterface
...
class UserService:
    ...
    async def refresh_session(
        ...
        refreshed_session = session.refresh(self.SESSION_EXPIRE_SECONDS)
    ...
    async def revoke_all_sessions(self, user_id: UserID) -> list[SessionAggregate]:
        ...
        revoked_sessions: list[SessionAggregate] = []
    ...
    async def verify_email(self, user_id: UserID, token: str) -> User:
    ...
    async def reset_password(
        ...
        if user.password_reset_token is None or not user.password_reset_token.is_valid():
            raise InvalidCredentialsError()
